...
function MobileTable({ datas, handleDeleteSubmit, openEditPopup, type }) {
    // Función para capitalizar el primer carácter de cada palabra
                    ...
                    <div className="border-t border-gray-200 dark:border-gray-700">
                        <dl>
                            {Object.entries(data).map(([key, value], idx) => (
                                key !== 'id' && (
                                    <div
                                        key={`${data.id}-${key}`} // Combina data.id con key para asegurar unicidad
                                        className={`${
                                            idx % 2 === 0 ? 'bg-gray-50 dark:bg-gray-700' : 'bg-white dark:bg-gray-800'
                                        } px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6`}
                                    >
                                        <dt className="text-sm font-medium text-gray-500 dark:text-gray-300 capitalize">{capitalizeFirstLetter(key)}</dt>
                                        <dd className="mt-1 text-sm text-gray-900 dark:text-white sm:mt-0 sm:col-span-2">
                                            {key === 'color' ? (
                                                <div
                                                    className="w-6 h-6 inline-block rounded-full border"
                                                    style={{ backgroundColor: value }}
                                                    title={value} // Muestra el color en el tooltip
                                                />
                                            ) : (
                                                value
                                            )}
                        ...
                        <div className="flex justify-between p-4 bg-gray-50 dark:bg-gray-700">
                            ...
                            <div className="flex items-center">
                                ...
                                <button 
                                    ...
                                    aria-label={`Eliminar ${type} con ID ${data.id}`}
                                >
                                    <DeleteIcon />
                                </button>
                            </div>
